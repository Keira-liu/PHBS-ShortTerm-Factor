...
def _parallel_evolve(n_programs, parents, X, y, panel, sample_weight, seeds, params):
    """Private function used to build a batch of programs within a job."""
    n_samples, n_features = X.shape[1], X.shape[2]
    # Unpack parameters
    ...
    while i < n_programs:
        ...
        if parents is None:
            ...
            if method < method_probs[0]:
                ...
                while True:
                    ...
                    if program_str not in programs_str:
        ...
        program.raw_fitness_ = program.raw_fitness(X, y, panel, curr_sample_weight)
        if max_samples < n_samples:
            # Calculate OOB fitness
            program.oob_fitness_ = program.raw_fitness(X, y,panel, oob_sample_weight)
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, panel, sample_weight=None):
       ...
       '''
        ...
        if isinstance(self.metric, _Fitness):
            ...
            if self.metric not in ('pearson', 'spearman', 'rank_ic', 'top_return'):
                raise ValueError('Unsupported metric: %s' % self.metric)
            self._metric = _fitness_map[self.metric]
        ...
        if isinstance(self, TransformerMixin):
            ...
            ##################
            evaluation = np.array([gp.execute(X, panel[-1])[:,-1] for gp in
