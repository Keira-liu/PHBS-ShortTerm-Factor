...
class _Program(object):
    ...
    def execute(self, X, p):
        ...
        for node in self.program:
            ...
            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:
                ...
                if function.type=='ts':
                    terminals = [np.reshape(np.repeat(t, 30 * X.shape[1]), (30, X.shape[1])).T if isinstance(t, float)
                                 else X[p - 30:p, :, t].T if isinstance(t, int)
    ...
    def raw_fitness(self, X, y, panel, sample_weight):
        ...
        for p in panel:
            y_pred = self.execute(X, p)
            if self.transformer:
                y_pred = self.transformer(y_pred)
            if len(y_pred.shape)>1:
                raw_fitness = self.metric(y[:,p], y_pred[:,-1], p, sample_weight)
            else:
                raw_fitness = self.metric(y[:, p], y_pred, p, sample_weight)
            raw_fitness_list.append(raw_fitness)

        print(self)
        print(abs(np.mean(raw_fitness_list)))
        return abs(np.mean(raw_fitness_list))
    ...
    def subtree_mutation(self, random_state):
        ...
        chicken = _Program(function_set=self.function_set,
