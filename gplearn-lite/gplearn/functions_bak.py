...
def _ts_sum(x1, window):
    result = np.sum(x1[:, -window:], axis=1)
    for i in range(1, x1.shape[1]-window):
        result = np.column_stack([np.sum(x1[:, -window-i:-i], axis=1), result])
    for i in range(x1.shape[1]-window, x1.shape[1]):
        result = np.column_stack([result[:,0], result])
    return result

def _ts_max(x1, window):
    result = np.max(x1[:, -window:], axis=1)
    for i in range(1, x1.shape[1]-window):
        result = np.column_stack([np.max(x1[:, -window-i:-i], axis=1), result])
    for i in range(x1.shape[1]-window, x1.shape[1]):
        result = np.column_stack([result[:,0], result])
    return result

def _ts_min(x1, window):
    result = np.min(x1[:, -window:], axis=1)
    for i in range(1, x1.shape[1]-window):
        result = np.column_stack([np.min(x1[:, -window-i:-i], axis=1), result])
    for i in range(x1.shape[1]-window, x1.shape[1]):
        result = np.column_stack([result[:,0], result])
    return result

def _ts_nanmean(x1, window):
    result = np.mean(x1[:, -window:], axis=1)
    for i in range(1, x1.shape[1]-window):
        result = np.column_stack([np.mean(x1[:, -window-i:-i], axis=1), result])
    for i in range(x1.shape[1]-window, x1.shape[1]):
        result = np.column_stack([result[:,0], result])
    return result

def _ts_prod(x1, window):
    result = np.prod(x1[:, -window:], axis=1)
    for i in range(1, x1.shape[1]-window):
        result = np.column_stack([np.prod(x1[:, -window-i:-i], axis=1), result])
    for i in range(x1.shape[1]-window, x1.shape[1]):
        result = np.column_stack([result[:,0], result])
    return result
